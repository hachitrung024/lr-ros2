...
The existing RGB dock then shows `/segmentation/overlay`. When no model path is
provided, segmentation is disabled and the RGB dock is remapped to the
original ZED RGB image. Terrain processing remains independent and does not
consume segmentation output.
