...
def _cache_process_exited(event, _context, *, cache_spec, pipeline):
    ...
    if event.returncode != 0:
        return _stop_launch(
            'Future-path cache process failed with return code ' f'{event.returncode}.'
        )
    if not cache_is_valid(cache_spec.path, cache_spec.identity):
        return _stop_launch(
            f'Future-path cache is invalid after preprocessing: ' f'{cache_spec.path}'
        )
    return [
        LogInfo(
            msg=TextSubstitution(
                text=(f'Future-path cache ready: {cache_spec.path}; ' 'starting SVO pipeline.')
            )
        ),
        *pipeline,
    ]
...
def _six_floats(value, name):
    try:
        ...
        raise ValueError(f'{name} must be a list of six finite numbers.') from exception
...
def launch_setup(context, *args, **kwargs):
    ...
    future_path_rebuild_cache = LaunchConfiguration('future_path_rebuild_cache')
    ...
    mavlink_match_tolerance_s = LaunchConfiguration('mavlink_match_tolerance_s')
    mavlink_max_gps_gap_s = LaunchConfiguration('mavlink_max_gps_gap_s')
    mavlink_body_to_camera = LaunchConfiguration('mavlink_body_to_camera')
    start_path_prediction_node = LaunchConfiguration('start_path_prediction_node')
    prediction_params_file = LaunchConfiguration('prediction_params_file')
    prediction_profile = LaunchConfiguration('prediction_profile')
    prediction_rover_config = LaunchConfiguration('prediction_rover_config')
    ...
    start_prediction_val = start_path_prediction_node.perform(context).lower()
    ...
    if mavlink_val and not svo_mode_val:
        return _stop_launch('mavlink:=true requires an SVO file; live mode is unsupported.')
    if mavlink_val and publish_svo_clock.perform(context).lower() != 'true':
        return _stop_launch('mavlink:=true requires publish_svo_clock:=true.')
    ...
    if mavlink_val:
        try:
            ...
            mavlink_max_gps_gap_val = float(mavlink_max_gps_gap_s.perform(context))
            if mavlink_match_tolerance_val < 0.0 or mavlink_max_gps_gap_val <= 0.0:
                raise ValueError
    ...
    if start_prediction_val == 'true' and prediction_profile_val == 'dynamic' and not mavlink_val:
        return _stop_launch(
    ...
    config_rviz2 = os.path.join(
        get_package_share_directory('lr_display_rviz2'), 'rviz2', 'zed_' + camera_type + '.rviz'
    )
    ...
    if camera_type == 'stereo' and start_segmentation_val != 'true':
        rviz_remappings.append(
            ('/segmentation/overlay', f'/{camera_name_val}/zed_node/rgb/color/rect/image')
        )
    ...
    zed_wrapper_launch = IncludeLaunchDescription(
        ...
        launch_arguments={
            ...
            'param_overrides': [
                ...
                TextSubstitution(
                    text=(
                        ';pos_tracking.pos_tracking_enabled:=false;' 'depth.depth_stabilization:=0'
                        if mavlink_val
                        else ''
                    )
                ),
    ...
    nodes = [rviz2_node, zed_wrapper_launch]
    ...
    if camera_type == 'stereo':
            ...
            parameters=[
                terrain_params_file,
                {
                    'input.point_cloud_topic': (
                        f'/{camera_name_val}/zed_node/' 'point_cloud/cloud_registered'
                    ),
        ...
        segmentation_node = Node(
            ...
            parameters=[
                ...
                {
                    'input.image_topic': (f'/{camera_name_val}/zed_node/' 'rgb/color/rect/image'),
        ...
        box_estimator_node = Node(
            ...
            parameters=[
                segmentation_params_file,
                {
                    ...
                    'input.depth_topic': (
                        f'/{camera_name_val}/zed_node/' 'depth/depth_registered'
                    ),
            ...
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource(
                    os.path.join(
                        ...
                        'launch',
                        'path_prediction.launch.py',
                    )
                ),
                launch_arguments={
                    'prediction_profile': prediction_profile,
                    'prediction_params_file': prediction_params_file,
                    'bridge_params_file': LaunchConfiguration('bridge_params_file'),
                    'prediction_runtime_params_file': LaunchConfiguration(
                        'prediction_runtime_params_file'
                    ),
                ...
                condition=IfCondition(TextSubstitution(text=start_prediction_val)),
            )
        )

    if not future_path_val:
        return [mavlink_exit_handler, *nodes] if mavlink_exit_handler is not None else nodes

    if not svo_mode_val:
        return _stop_launch('future_path:=true requires an SVO file; live mode is ' 'unsupported.')
    ...
    rebuild_cache = future_path_rebuild_cache.perform(context).lower() == 'true'
    if cache_is_valid(cache_spec.path, cache_spec.identity) and not rebuild_cache:
        return [
            LogInfo(msg=TextSubstitution(text=f'Future-path cache hit: {cache_spec.path}')),
            *nodes,
        ]
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument(
                ...
                default_value='',
                description=(
                    'Cache root. Empty stores .lr_future_path_cache beside ' 'the SVO file.'
                ),
            ),
            DeclareLaunchArgument(
            ...
            DeclareLaunchArgument(
                ...
                default_value='60.0',
                description=(
                    'Maximum difference between SVO and MAVLink session ' 'start times in seconds.'
                ),
            ),
            DeclareLaunchArgument(
            ...
            DeclareLaunchArgument(
                ...
                default_value='true',
                description=(
                    'Start the LR terrain geometry estimator for stereo ' 'camera models.'
                ),
                choices=['true', 'false'],
            ),
            DeclareLaunchArgument(
            ...
            DeclareLaunchArgument(
                'segmentation_params_file',
                default_value=os.path.join(
                    get_package_share_directory('lr_segmentation'), 'config', 'segmentation.yaml'
                ),
            ...
            DeclareLaunchArgument(
                ...
                default_value='',
                description=('External Ultralytics instance-segmentation checkpoint.'),
            ),
            DeclareLaunchArgument(
            ...
            DeclareLaunchArgument(
                'bridge_params_file',
                default_value=os.path.join(
                    get_package_share_directory('lr_prediction_bridge'), 'config', 'bridge.yaml'
                ),
            ),
            DeclareLaunchArgument(
                'prediction_runtime_params_file',
