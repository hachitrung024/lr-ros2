...
from .core import GridMapSampler, StepPrediction, TerrainSample
...
class CanonicalPredictionVisualizerNode(Node):
    ...
    def __init__(self, **node_kwargs) -> None:
        ...
        self._trajectory_topic = str(self.declare_parameter(
            "input.trajectory_topic", "/trajectory"
        ).value)
        self._geometry_topic = str(self.declare_parameter(
            "input.geometry_topic", "/geometry"
        ).value)
        self._prediction_topic = str(self.declare_parameter(
            "input.prediction_topic", "/predict_output"
        ).value)
        ...
        self._collision_warning_height_m = float(self.declare_parameter(
            "visualization.collision_warning_height_m", 1.0
        ).value)
        self._collision_warning_triangle_size_m = float(
            self.declare_parameter(
                "visualization.collision_warning_triangle_size_m", 0.9
            ).value
        )
        self._collision_warning_line_width_m = float(
            self.declare_parameter(
                "visualization.collision_warning_line_width_m", 0.08
            ).value
        )
        self._collision_warning_symbol_scale_m = float(
            self.declare_parameter(
                "visualization.collision_warning_symbol_scale_m", 0.60
            ).value
        )
        ...
        self._steps_publisher = self.create_publisher(
            DiagnosticArray, steps_topic, output_qos
        )
        self._markers_publisher = self.create_publisher(
            MarkerArray, markers_topic, output_qos
        )
    ...
    def _validate_parameters(
        self, steps_topic: str, markers_topic: str
    ) -> None:
        topics = (
            self._trajectory_topic,
            self._geometry_topic,
    ...
    def _on_prediction(self, message: PredictionOutput) -> None:
        ...
        if trajectory is None:
            self._warn(
                f"Prediction cycle {cycle_id} has no cached trajectory"
            )
            return
        ...
        self._markers_publisher.publish(predictions_to_markers(
            ...
            collision_warning_triangle_size_m=(
                self._collision_warning_triangle_size_m
            ),
            collision_warning_line_width_m=(
                self._collision_warning_line_width_m
            ),
            collision_warning_symbol_scale_m=(
                self._collision_warning_symbol_scale_m
            ),
        ))
    ...
    def _join(
        ...
        geometry_by_id = {
            ...
            for item in ([] if geometry is None else geometry.steps)
        }
        rollover_by_id = {
            int(item.step_id): item for item in output.rollover_steps
        }
        collision_by_id = {
            int(item.step_id): item for item in output.collision_steps
        }
        predictions = []
        distance = 0.0
        previous_xy = None
        for step_index, step in enumerate(trajectory.steps, start=1):
            ...
            if previous_xy is not None:
                distance += math.hypot(
                    xy[0] - previous_xy[0], xy[1] - previous_xy[1]
                )
            previous_xy = xy
            terrain = self._terrain_for_step(
                xy[0], xy[1], geometry_by_id.get(int(step.step_id))
            )
            collision = collision_by_id.get(int(step.step_id))
            candidates = (
                [] if collision is None else collision.collision_objects
            )
            ...
            elevation = (
                terrain.elevation_m if terrain.valid else 0.0
            )
            predictions.append(StepPrediction(
                ...
                predicted_roll_deg=(
                    math.nan if rollover is None
                    else float(rollover.predicted_roll_deg)
                ),
                predicted_pitch_deg=(
                    math.nan if rollover is None
                    else float(rollover.predicted_pitch_deg)
                ),
                static_stability_margin_m=(
                    math.nan if rollover is None
                    else float(rollover.static_stability_margin_m)
                ),
                ...
                stability_moment_valid=bool(
                    moment is not None and moment.valid
                ),
                minimum_stability_moment_nm=(
                    math.nan if moment is None or not moment.valid
                ...
                zmp_xy=(
                    None if zmp is None or not zmp.valid
                    else (float(zmp.x), float(zmp.y))
                ),
                zmp_margin_m=(
                    math.nan if zmp is None or not zmp.valid
                    else float(zmp.margin_m)
                ),
                normalized_zmp_margin=(
                    math.nan if zmp is None or not zmp.valid
                    else float(zmp.normalized_margin)
                ),
                nearest_zmp_edge=(
                    "" if zmp is None or not zmp.valid
                    else str(zmp.nearest_edge)
                ),
            ))
        return predictions
    ...
    def _on_time_jump(self, _time_jump) -> None:
        ...
        self._markers_publisher.publish(MarkerArray(markers=[Marker(
            header=header,
