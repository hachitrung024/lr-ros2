"""ROS 2 node that publishes an RGB segmentation overlay."""
...
from .model import create_overlay_model


class SegmentationNode(Node):
    """Run segmentation for each received image and publish its overlay."""

    def __init__(
        self,
        *,
        parameter_overrides=None,
        model_factory=create_overlay_model,
    ) -> None:
        ...
        self.get_logger().info(
            f"Publishing RGB segmentation overlay from {image_topic}"
        )

    def _on_image(self, message: Image) -> None:
        try:
            image_bgr = image_message_to_bgr(message)
            overlay_bgr = self._model.predict_overlay(image_bgr)
            overlay = bgr_to_image_message(overlay_bgr, message.header)
