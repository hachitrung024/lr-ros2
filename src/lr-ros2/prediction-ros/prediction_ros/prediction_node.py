...
class PredictionNode:
    ...
    def _configure(self, node) -> None:
        ...
        self._reuse_latest_inputs = bool(
            node.get_parameter("reuse_latest_inputs_per_cycle").value
        )
    ...
    def _trajectory_callback(self, msg) -> None:
        try:
            ...
            if (
                self._reuse_latest_inputs
                and self._latest_objects_msg is not None
            ):
                latest = self._latest_objects_msg
                self._publish_if_ready(
                    self.runtime.on_objects(
                        RosAdapters.objects_from_ros(latest),
                        frame_id=latest.header.frame_id,
                    )
                )
            if self._reuse_latest_inputs and self._latest_state_msg is not None:
                latest = self._latest_state_msg
                self._publish_if_ready(
                    self.runtime.on_state(
                        RosAdapters.state_from_ros(latest),
                        frame_id=latest.header.frame_id,
                    )
                )
        except Exception:
